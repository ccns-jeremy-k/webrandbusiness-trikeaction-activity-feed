...
class BuddyBoss_Media_Photo_Hooks {
	...
	public function activity_photo_size() {
		if ( ! $this->activity_photo_size ) {
			...
			if ( ! $this->activity_photo_size ) {
				$this->activity_photo_size = 'large';
			}
		}
	...
	public function bp_activity_after_save( &$activity ) {
		...
		if ( $user && isset( $_POST['pics_uploaded'] ) && ! empty( $_POST['pics_uploaded'] ) ) {
	...
	public function bp_get_activity_action( $action ) {
		...
		if ( $buddyboss_media_action &&
			...
			$time_since = apply_filters_ref_array( 'bp_activity_time_since', array(
				'<span class="time-since">' . bp_core_time_since( $activities_template->activity->date_recorded ) . '</span>',
				&$activities_template->activity
			) );

			// Insert the permalink
			if ( ! bp_is_single_activity() ) {
				$content = apply_filters_ref_array( 'bp_activity_permalink', array(
					sprintf( '%1$s <a href="%2$s" class="view activity-time-since" title="%3$s">%4$s</a>', $content, bp_activity_get_permalink( $activities_template->activity->id, $activities_template->activity ), esc_attr__( 'View Discussion', 'buddyboss-media' ), $time_since ),
					&$activities_template->activity
				) );
			} else {
	...
	public function media_container_activity_content( $act_obj, $content ) {
		...
		if ( $type === 'photo' && ! empty( $media_ids ) ) {
			...
			if ( buddyboss_media_check_custom_activity_template_load() ) {
				foreach ( $media_ids as $media_id ) {
					$image = wp_get_attachment_image_src( $media_id, 'large' );

					if ( ! empty( $image ) && is_array( $image ) && count( $image ) > 2 ) {
						$src = $image[0];

						$full = wp_get_attachment_image_src( $media_id, 'full' );
						$w    = ! empty( $full[1] ) ? $full[1] : 600;
						$h    = ! empty( $full[2] ) ? $full[2] : 600;
						...
						if ( $full !== false && is_array( $full ) && count( $full ) > 2 ) {
							$content .= '<a data-caption="'.esc_attr( $media_title ).'" class="buddyboss-media-photo-wrap" href="' . esc_url( $full[0] ) . '" data-activity-id="' . $act_id . '" data-id="' . esc_attr( $media_id ) . '" data-width="' . esc_attr( $w ) . '" data-height="' . esc_attr( $h ) . '">';
							$content .= '<img title="' . esc_attr( $media_title ) . '" data-permalink="' . bp_get_activity_thread_permalink() . '" data-photo-id="' . $media_id . '" class="buddyboss-media-photo" src="' . $src . '"' . $width_markup . ' ' . $alt . ' data-comment-count="' . $comment_count . '" data-favorite-count="' . $favorite_count . '" data-bbmfav="' . $data_fav . '" data-media="' . $act_id . '" data-owner="' . $owner . '"/></a>';
						} else {
							$content .= '<a data-caption="'.esc_attr( $media_title ).'" class="buddyboss-media-photo-wrap" href="' . esc_url( $src ) . '" data-activity-id="' . $act_id . '" data-id="' . esc_attr( $media_id ) . '" data-width="' . esc_attr( $w ) . '" data-height="' . esc_attr( $h ) . '">';
							$content .= '<img title="' . esc_attr( $media_title ) . '" data-permalink="' . bp_get_activity_thread_permalink() . '" data-photo-id="' . $media_id . '" data-comment-count="' . $comment_count . '" data-favorite-count="' . $favorite_count . '" data-bbmfav="' . $data_fav . '" data-media="' . $act_id . '" data-owner="' . $owner . '" class="buddyboss-media-photo" src="' . $src . '"' . $width_markup . ' ' . $alt . ' />';
						}
						$content .= '</div>';
					}
				}
				...
				$w1     = isset( $image1[1] ) ? $image1[1] : 0;
				$h1     = isset( $image1[2] ) ? $image1[2] : 0;

				$image2 = wp_get_attachment_image_src( $media_ids[1], 'full' );
				$w2     = isset( $image2[1] ) ? $image2[1] : 0;
				$h2     = isset( $image2[2] ) ? $image2[2] : 0;
...
//					}
					...
					if ( ! empty( $image ) && is_array( $image ) && count( $image ) > 2 ) {
						...
						$owner = ( $act_obj->user_id == get_current_user_id() ) ? '1' : '0';
						$all_imgs_html .= '<div class="photo-item">';
						...
						$caption_inner .= '<div class="buddyboss_media_caption_body">' . $act_obj->content . '</div>';
						$caption       = '<div class="buddyboss_media_caption" >' . $caption_inner . '</div>';
						$all_imgs_html .= $caption;

						if ( $full !== false && is_array( $full ) && count( $full ) > 2 ) {

							if ( empty ( $maybe_display_none ) ) {
								$all_imgs_html .= '<a data-caption="'.esc_attr( $media_title ).'" data-activity-id="' . $act_id . '" data-width="' . esc_attr( $w ) . '" data-height="' . esc_attr( $h ) . '" data-id="' . esc_attr( $media_id ) . '" class="buddyboss-media-photo-wrap size-' . $filenames[ $img_counter ] . '" ' . $height_markup . '  href="' . $full[0] . '" ' . $maybe_display_none . '>';
							} else {
								$all_imgs_html .= '<a data-caption="'.esc_attr( $media_title ).'" data-activity-id="' . $act_id . '" data-width="' . esc_attr( $w ) . '" data-height="' . esc_attr( $h ) . '" data-id="' . esc_attr( $media_id ) . '" class="buddyboss-media-photo-wrap" ' . $height_markup . '  href="' . $full[0] . '" ' . $maybe_display_none . '>';
							}
							...
							if ( 4 > $img_counter ) {
								$all_imgs_html .= '<img title="' . esc_attr( $media_title ) . '" data-photo-id="' . $media_id . '" data-permalink="' . bp_get_activity_thread_permalink() . '" class="buddyboss-media-photo" src="' . $src . '"' . $width_markup . ' ' . $alt . ' data-comment-count="' . $comment_count . '" data-favorite-count="' . $favorite_count . '" data-bbmfav="' . $data_fav . '" data-media="' . $act_id . '" data-owner="' . $owner . '"/>';
							} else {
								$all_imgs_html .= '<img title="' . esc_attr( $media_title ) . '" data-photo-id="' . $media_id . '" data-permalink="' . bp_get_activity_thread_permalink() . '" class="buddyboss-media-photo" data-comment-count="' . $comment_count . '" data-favorite-count="' . $favorite_count . '" data-bbmfav="' . $data_fav . '" data-media="' . $act_id . '" data-owner="' . $owner . '"/>';
							}

							$all_imgs_html .= '</a>';
						} else {
							$all_imgs_html .= '<img title="' . esc_attr( $media_title ) . '" ' . $maybe_display_none . ' data-photo-id="' . $media_id . '" data-permalink="' . bp_get_activity_thread_permalink() . '" data-comment-count="' . $comment_count . '" data-favorite-count="' . $favorite_count . '" data-bbmfav="' . $data_fav . '" data-media="' . $act_id . '" data-owner="' . $owner . '" class="buddyboss-media-photo size-' . $filesizes[ $img_counter ] . '" src="' . $src . '"' . $width_markup . ' ' . $alt . ' />';
						}
				...
				$content .= "<div class='bbmedia-grid-wrapper buddyboss-media-photos-wrap-container'>" . $all_imgs_html . "</div>";
			}
		}
